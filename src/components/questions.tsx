  ...
  return (
    <div key={idxx} className="w-[97%] lg:w-[90%] mx-auto">
      <Accordion type="single" collapsible>
        <AccordionItem value={`item-${idxx}`} >
          <AccordionTrigger onClick={toggleOpen} className={`px-7 lg:px-8 border border-gray-300 ${(paquete || mobile) ? "rounded-2xl" : "rounded-none"} ${paquete ? "font-bold" : "font-normal"} py-4 ${(paquete || !mobile) ? mobile ? "text-[12px]" : "" : "text-[12px]"} w-full ${(paquete) ? "text-gray-600" : "text-[#000000]"} flex justify-between text-left ${(paquete || mobile) ? "bg-gray-200 capitalize" : "bg-white"}`}>
            <div className="flex flex-row gap-x-3">
              {(!paquete && !mobile) && <Image src="/nubesita.svg" alt="aa" width="25" height="25" />}
              {(!paquete || mobile) && (idxx + " .")}
              {question}
            </div>
            {open ? <ChevronDownIcon className="w-3 inline-block" /> : <ChevronUpIcon className="w-3 inline-block" />}
          </AccordionTrigger>
          <AccordionContent>
            <div dangerouslySetInnerHTML={{ __html: answer }} className="tourQWERTY" />
          </AccordionContent>
