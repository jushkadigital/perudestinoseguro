...
export function NavbarDefault({ lng, destinos }: Props) {
  ...
  const navList = navNames.map((ele, idx) => 
    ele === t('destinations') ? (
      <DropdownMenu key={idx}>
        ...
        <DropdownMenuContent>
          {destinos.map(ele => (
            <DropdownMenuItem key={ele}>
              <Link 
                onClick={() => setIdxNav(2)} 
                className="w-full block text-center" 
                href={`/${lng}/${t('destinations')}/?city=${ele.toLowerCase()}`}
              >
                {ele}
              </Link>
            </DropdownMenuItem>
  ...
  return (
    <nav className="w-full py-3 lg:px-0 lg:py-0">
      <div className="container mx-auto flex items-center justify-center lg:justify-between text-blue-gray-900">
        <div className="flex w-2/3 lg:w-3/12 flex-row justify-end lg:justify-center">
          <Link href="/" className="" onClick={() => setIdxNav(1)}>
            <img src="/pdsLogo.png" alt="logo-ct" className="w-[100px]" />
        ...
        <div className="hidden lg:block flex mb-3">
          ...
          <motion.div 
            ...
            animate="visible" 
            variants={variants} 
            className="absolute w-28 h-1 bg-red-800 mx-2 mb-3" 
          />
        </div>
        <div className="flex items-center gap-x-1 w-1/12">
          <Link href="/api/auth/signin">
            <Button variant="ghost" size="lg" className="hidden lg:inline-block text-[#D20000]">
              <UserCircle className="w-5 h-5 mr-2" /> Entrar
            </Button>
          </Link>
          <Link href={`/${lng}/${t('contact')}`}>
            <Button variant="default" size="lg" className="hidden lg:inline-block bg-[#D20000] text-white">
              {t('contact')}
            </Button>
          </Link>
        </div>
          ...
          <Sheet>
            ...
            <SheetContent>
              <div className="flex flex-col space-y-4 mt-4">
                <Link href="/api/auth/signin">
                  <Button variant="ghost" size="lg" className="w-full text-[#D20000]">
                    <UserCircle className="w-5 h-5 mr-2" /> Entrar
                  </Button>
                </Link>
                <Link href={`/${lng}/${t('contact')}`}>
                  <Button variant="default" size="lg" className="w-full bg-[#D20000] text-white">
                    {t('contact')}
                  </Button>
                </Link>
                {navList}
